            ...
            <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div ref={menuRef} className="md:hidden relative flex items-center">
                        ...
                        {isMobileMenuOpen && (
                            <div className={`absolute top-full right-0 mt-2 w-56 origin-top-right rounded-md shadow-lg focus:outline-none transition-colors
                                ${isLight
                                    ? 'bg-white ring-1 ring-black ring-opacity-5'
                                    : 'bg-gray-900/90 backdrop-blur-sm ring-1 ring-white/10'
                                }`}>
                                <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="mobile-menu-button">
                                    ...
                                    {/* Divider */}
                                    <div className={`border-t my-1 ${isLight ? 'border-gray-100' : 'border-white/10'}`}></div>

                                    {/* Mobile Language Switcher */}
                                    <div className="px-4 py-2 flex items-center justify-center space-x-2 text-base font-medium">
                                        {currentLang === 'ID' ? (
                                            <span className={`font-bold ${isLight ? 'text-stone-900' : 'text-white'}`}>ID</span>
                                        ) : (
                                            <button onClick={() => onLangChange('ID')} className={`transition-colors ${isLight ? 'text-stone-600 hover:text-stone-900' : 'text-gray-300 hover:text-white'}`}>ID</button>
                                        )}
                                        <span className={`transition-colors ${isLight ? 'text-stone-300' : 'text-white/50'}`}>|</span>
                                        {currentLang === 'EN' ? (
                                            <span className={`font-bold ${isLight ? 'text-stone-900' : 'text-white'}`}>EN</span>
                                        ) : (
                                            <button onClick={() => onLangChange('EN')} className={`transition-colors ${isLight ? 'text-stone-600 hover:text-stone-900' : 'text-gray-300 hover:text-white'}`}>EN</button>
                                        )}
                                    </div>
                                </div>
                            </div>
